...
-- 4 rows
-- Shantel	Johnson	205	2023-06-28	2023-07-02
...
-- The results should include all rooms, regardless if there is a reservation associated with the room.
--------------------
SELECT
	Room.RoomId,
    ...
    CASE
		WHEN (((Room.RoomId BETWEEN '201' AND '204') OR (Room.RoomId BETWEEN '301' AND '304')) AND Reservation.Adults <= 2)
			THEN ((Room.BaseRate) * DATEDIFF(Reservation.CheckOut, Reservation.CheckIn))
		 WHEN (((Room.RoomId BETWEEN '201' AND '204') OR (Room.RoomId BETWEEN '301' AND '304')) AND Reservation.Adults > 2)
			 THEN ((Room.BaseRate + ((Reservation.Adults - RoomType.StandardOccupancy) * 10) * DATEDIFF(Reservation.CheckOut, Reservation.CheckIn)))
		...
		WHEN (((Room.RoomId BETWEEN '401' AND '402')) AND Reservation.Adults <= 3)
			THEN ((Room.BaseRate) * DATEDIFF(Reservation.CheckOut, Reservation.CheckIn))
		WHEN (((Room.RoomId BETWEEN '401' AND '402')) AND Reservation.Adults > 3)
			THEN ((Room.BaseRate + ((Reservation.Adults - RoomType.StandardOccupancy) * 20) * DATEDIFF(Reservation.CheckOut, Reservation.CheckIn)))
	END AS Total
FROM Guest
...
RIGHT OUTER JOIN ReservationRoom ON Reservation.ReservationId = ReservationRoom.ReservationId
RIGHT OUTER JOIN Room ON ReservationRoom.RoomId = Room.RoomId;
...
WHERE 	(Reservation.Adults + Reservation.Children) > 2
		AND ((Reservation.CheckIn BETWEEN '2023-04-01' AND '2023-04-30')
